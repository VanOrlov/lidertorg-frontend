<template>
    <aside class="catalog-sidebar">
        <div class="sidebar-categories-container">
            <div class="parent-container" v-if="categories.length !== 0" v-for="category in categories" :id="category.id" @mouseover="showMenu(category.id)" @mouseleave="hideMenu">
                <NuxtLink :to="'http://localhost:3000/category/' + category.slug" class="parent-category">
                    <span>{{ category.name }}</span>
                    <span v-if="category.childrens.length !== 0" class="aside-item-arrow">></span>
                </NuxtLink>
                <div v-if="category.childrens.length !== 0" class="category-child-right-dropdown" :id="category.id" :class="{ visible: activeMenu === category.id }">
                    <NuxtLink :to="'http://localhost:3000/category/' + child.slug" v-for="child in category.childrens" class="category-child-item">
...
const activeMenu: Ref<number | null> = ref(6);
